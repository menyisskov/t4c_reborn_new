...
public class GuiText extends AbstractGuiElement {
    ...
    public GuiText(BitmapFont font, float x, float y, Supplier<String> textSupplier, Supplier<Color> colorSupplier) {
        super(x, y);
    ...
    public static GuiText header(String text, float x, float y) {
        return new GuiText(
                FontManager.getInstance().getHaettenschweilerFont(18, T4C_GOLD),
                x,
                y,
                () -> text
        );
    ...
    /** Creates a standard GUI information label with explicit presentation. */
    public static GuiText information(Supplier<String> text, float x, float y, int fontSize, Color color) {
        return new GuiText(
                FontManager.getInstance().getJetBrainsMonoFont(fontSize, color),
                x,
                y,
                text,
                () -> color
    ...
    public void renderOutlined(SpriteBatch batch, Color textColor, Color outlineColor, float outlineOffset) {
        String text = textSupplier.get();
        if (text == null || text.isEmpty()) {
            return;
        }
    ...
    public boolean contains(float screenX, float screenY) {
        // Texte bitmap: y est la ligne de base; ses bornes sont donc inversées
        // par rapport aux widgets y-down. Ce contrat est utilisé par le Ctrl+drag.
        String text = textSupplier.get();
        if (text == null || text.isEmpty()) {
            return false;
        }
