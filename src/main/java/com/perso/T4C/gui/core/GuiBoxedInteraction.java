...
/**
 * Generic move/resize controller for every boxed GUI element.
 * Ctrl+drag moves; Shift+drag resizes from the nearest edge.
 */
public final class GuiBoxedInteraction {
    ...
    public boolean dragged(float sx, float sy) {
        ...
        if (left) { x = Math.min(sx, fixedRight - MIN_SIZE); w = fixedRight - x; }
        else if (right) w = Math.max(MIN_SIZE, sx - x);
        if (top) { y = Math.min(sy, fixedBottom - MIN_SIZE); h = fixedBottom - y; }
    ...
    public boolean touchUp() {
        ...
        try {
            Gdx.app.log("GuiScreen", String.format(Locale.ROOT,
                    ...
                    active.getX(), active.getY(), active.getWidth(), active.getHeight()));
    ...
    private void selectNearestEdge(GuiResizable e, float sx, float sy) {
        ...
        left = nearest == l; right = !left && nearest == r;
        top = !left && !right && nearest == t; bottom = !left && !right && !top;
