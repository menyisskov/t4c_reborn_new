...
/** Merchant panel for repairing individual items or all damaged equipment. */
public final class RepairScreen extends GuiListScreen {
    ...
    public RepairScreen(Player player) {
        this.player = player;
        try { background = SpriteLoader.getInstance().getRegionFromSpriteName("GUIBackBuy"); }
        catch (GameException ignored) { background = null; }
        centerOnScreen();
    ...
    private void loadEntries() {
        ...
        for (int i = 0; i < player.getInventory().size(); i++) {
            ...
            if (ItemDurabilityService.repairCost(item, durability) > 0) entries.add(RepairEntry.inventory(item, i, durability));
        }
        ...
        for (BodyPart slot : player.getEquippedItems().keySet()) {
            ...
            if (ItemDurabilityService.repairCost(item, durability) > 0) entries.add(RepairEntry.equipped(item, primary, durability));
        }
    }

    @Override protected void rebuildList() {
        labels.removeAll(dynLabels); dynLabels.clear();
        buttons.removeAll(dynButtons); dynButtons.clear(); zones.clear(); animatedSprites.clear();
        ...
        for (int i = start; i < end; i++) {
            ...
            if (icon != null) animatedSprites.add(new GuiAnimatedSprite(List.of(icon), x + ICON_BOX[0],
                    y + rowY + ICON_BOX[1], 1f).boxed(ICON_BOX[2], ICON_BOX[3]));
            ...
            addDyn(entry.durability < 25 ? FontManager.getInstance().getJetBrainsMonoFont(12, Color.RED) : white,
                    CELL_THIRD, rowY, entry::thirdColumnText, entry.durability < 25 ? Color.RED : WHITE);
            zones.add(new GuiClickZone(x + 10f, y + rowY - 4f, 360f, ROW_H_PITCH - 4f,
                    () -> { selected = entry; rebuildList(); }));
        }
        ...
        addDyn(gold, TOTAL_VAL_BOX, 0, () -> String.valueOf(Math.max(0, player.getGold() - cost)), GOLD);
    ...
    private void addActionButton(boolean enabled) {
        ...
        GuiButton button = new GuiButton(normal, normal, normal, x + ACTION_BTN_X, y + ACTION_BTN_Y,
                enabled ? this::repair : null).withLabel(
        ...
        buttons.add(button); dynButtons.add(button);
    }

    private void repair() {
        boolean success = selected == null ? ItemDurabilityService.repairAll(player)
        ...
        if (!success) { SystemMessage.showShared(I18n.message("message.not_enough_gold")); return; }
        PlayerStateStore.save(player); selected = null; loadEntries(); rebuildList();
    }

    @Override protected List<? extends ListRow> rows() { return entries; }
    @Override protected void basketAdd(ListRow row) { selected = (RepairEntry) row; rebuildList(); }
    @Override protected void basketRemove(ListRow row) { selected = null; rebuildList(); }

    private static final class RepairEntry implements ListRow {
        final ItemDefinition item; final int inventoryIndex; final BodyPart slot; final double durability;
        private RepairEntry(ItemDefinition item, int index, BodyPart slot, double durability) {
            this.item = item; this.inventoryIndex = index; this.slot = slot; this.durability = durability;
        }
        static RepairEntry inventory(ItemDefinition item, int index, double durability) { return new RepairEntry(item, index, null, durability); }
        static RepairEntry equipped(ItemDefinition item, BodyPart slot, double durability) { return new RepairEntry(item, -1, slot, durability); }
        long cost() { return ItemDurabilityService.repairCost(item, durability); }
        @Override public String nameText() { return I18n.resolve(item.getName()); }
        @Override public String priceText() { return String.valueOf(cost()); }
        @Override public String thirdColumnText() { return ItemDurabilityService.format(durability) + "%"; }
        @Override public String iconSprite() { return ItemIconRegistry.iconFor(item); }
