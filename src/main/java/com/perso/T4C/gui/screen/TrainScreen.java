...
/**
 * Training screen opened when the player clicks a NPC's dialog keyword
 * ({@code KeywordActionType.TRAIN}). Lists trainable stats/skills with their
 * cost per point; clicking +1 spends gold and increments the stat or skill.
 *
 * <p>Supports both primary stats (strength, dexterity, …) and discrete T4C
 * skills (attack, dodge, archery, stun_blow, …) stored in
 * {@code player.getSkills()}.
 */
public class TrainScreen extends GuiScreenBase {
    ...
    private void addRow(BitmapFont font, float lx, float ly,
                        java.util.function.Supplier<String> text, Color color) {
        GuiText label = color == null
