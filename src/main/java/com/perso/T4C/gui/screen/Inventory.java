 ...
 */
    ...
    private void loadBackgrounds() {
        backgroundTop = loadFirstExisting("GUI_InvBackT", "GUI_InvBackTop", "GUI_BackInvT", "GUI_BackInventoryTop");
        backgroundBottom = loadFirstExisting("GUI_InvBackB", "GUI_InvBackBottom", "GUI_BackInvB", "GUI_BackInventoryBottom");

        // Some packs ship a single inventory background instead of split top/bottom.
        ...
        if (backgroundTop == null && backgroundBottom == null) {
            log.warn("Inventory background not found. Tried known sprite names: {}",
                    Arrays.asList(
                            "GUI_InvBackT", "GUI_InvBackTop", "GUI_BackInvT", "GUI_BackInventoryTop",
        ...
        if (started != null) {
            ...
            if (started.source == DragSource.INVENTORY) {
                if (now - lastClickTimeMs < 300 && started.inventoryIndex == lastClickInventoryIndex) {
                    // A double-click is a complete action of its own. Never let
                    // a drag left by the first click be processed on mouse-up.
                    dragState = null;
    ...
    private void autoEquipItem(DragState drag) {
        ...
        if (target == null) {
            ItemUseService.Result use = ItemUseService.useOnSelf(player, drag.itemName,
    ...
    public void onMouseMove(float screenX, float screenY) {
        ...
        // The original client shows the item name and stack size in the yellow
        // inventory information field while hovering, without drawing a number
        // over the icon itself.
    ...
    private void showItemHoverText(float screenX, float screenY) {
        ...
        if (inv == null) { clearHoverItemText(); return; }
        GuiInventory.ItemHit hit = inv.hitTest(screenX, screenY);
        if (hit == null) { clearHoverItemText(); tooltip.clear(); return; }
        int count = 0;
    ...
    private void addStatLabels() {
        ...
        var gold = Color.valueOf("F2B705");
        // Stat value boxes on the right column (x 352-399).
        labels.add(new GuiBoxedText(font, x + 352f, y + 96f, 48f, 17f, () -> String.valueOf(player.getStrength()), () -> Color.WHITE));
        labels.add(new GuiBoxedText(font, x + 352f, y + 130f, 48f, 17f, () -> String.valueOf(player.getEndurance()), () -> Color.WHITE));
        labels.add(new GuiBoxedText(font, x + 352f, y + 164f, 48f, 17f, () -> String.valueOf(player.getDexterity()), () -> Color.WHITE));
        labels.add(new GuiBoxedText(font, x + 352f, y + 198f, 48f, 17f, () -> String.valueOf(player.getWisdom()), () -> Color.WHITE));
        labels.add(new GuiBoxedText(font, x + 352f, y + 232f, 48f, 17f, () -> String.valueOf(player.getIntelligence()), () -> Color.WHITE));
        labels.add(new GuiBoxedText(font, x + 352f, y + 51f, 48f, 17f,
        ...
        labels.add(new GuiBoxedText(infoFont, x + 206f, y + 397f, 248f, 18f,
                () -> hoverItemText, () -> gold));
    }
    ...
    private void addPlayerParts() {
        ...
        // Equipment slot zones {left, top, w, h} measured on GUI_InvBackT; the
        // equipped item's icon is centered inside the zone.
    ...
    private DragState tryStartDragFromPart(float screenX, float screenY) {
        ...
        for (int i = playerParts.size() - 1; i >= 0; i--) {
            ...
            if (region == null) {
                String fallback = player.getAnimations() == null ? null : player.getAnimations().getPartMap().get(bodyPart);
    ...
    private void handleDrop(float screenX, float screenY, DragState drag) {
        ...
        if (targetPart != null) {
            ...
            boolean movingRing = drag.source == DragSource.EQUIPPED
            ...
            if (drag.source == DragSource.INVENTORY) {
                ...
                // Moving between equipment slots is only valid for interchangeable rings.
                ...
                if (displaced != null && !displaced.equals(drag.itemName)) player.getInventory().add(displaced);
            }
        ...
        if (inventoryTarget != null) {
            ...
            // Releasing an inventory item over the inventory is a cancelled
            // drag, not a request to drop the item into the game world.
            return;
        }

        // The whole inventory window is a safe drop zone. Releasing over the
        // character preview, statistics, decoration or an empty equipment slot
        // cancels the drag; only coordinates outside the window drop to ground.
        if (isInsideInventoryWindow(screenX, screenY)) {
            return;
        }

        // Dropping outside the inventory/equipment places the item in the world.
        if (drag.source == DragSource.INVENTORY) {
    ...
    private GuiPlayerPart findPlayerPartDropTarget(float screenX, float screenY, float fallbackW, float fallbackH) {
        for (int i = playerParts.size() - 1; i >= 0; i--) {
            GuiPlayerPart part = playerParts.get(i);
            float w = part.getWidth() > 0f ? part.getWidth() : fallbackW;
            float h = part.getHeight() > 0f ? part.getHeight() : fallbackH;
            if (screenX >= part.getX() && screenX <= part.getX() + w
                    && screenY >= part.getY() && screenY <= part.getY() + h) {
                return part;
            }
        }
        return null;
    ...
    private static String formatBodyPart(BodyPart bodyPart) {
        return bodyPart == null ? null : bodyPart.name();
    }
    ...
    private boolean showArmorClassTooltipAt(float screenX, float screenY) {
        ...
        // The AC value is drawn at x+369/y+54. Keep a comfortably clickable
        // rectangle around the icon/value, matching item right-click behavior.
        if (screenX < x + 340f || screenX > x + 415f
                || screenY < y + 35f || screenY > y + 78f) {
            return false;
        }
