...
public abstract class GuiListScreen extends GuiScreenBase {
    ...
    protected GuiBoxedText boxed(BitmapFont font, float[] box, float dy,
                                 Supplier<String> text, Color color) {
        return new GuiBoxedText(font, x + box[0], y + box[1] + dy,
                box[2], box[3], text, () -> color);
    }

    protected void addDyn(BitmapFont font, float[] box, float dy,
                          Supplier<String> text, Color color) {
