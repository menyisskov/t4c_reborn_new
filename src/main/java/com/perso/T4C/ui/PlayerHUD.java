...
public class PlayerHUD {
  ...
  private void loadTopBarRegions() throws GameException {
    ...
    topBarBackground =
        subRegion(
            chatBackground,
            TOP_BAR_BACKGROUND_X,
            TOP_BAR_BACKGROUND_Y,
            TOP_BAR_BACKGROUND_WIDTH,
            TOP_BAR_BACKGROUND_HEIGHT);
    topBarBorder =
        subRegion(
            chatBackground,
            TOP_BAR_BORDER_X,
            ...
            TOP_BAR_BORDER_SOURCE_HEIGHT);
  }

  private static TextureRegion subRegion(
      TextureRegion parent, int x, int y, int width, int height) {
    return new TextureRegion(
        parent.getTexture(), parent.getRegionX() + x, parent.getRegionY() + y, width, height);
  }
        ...
        () -> {
          if (generatedTopBar != null) {
            ...
            drawTiled(
                ...
                topBarBorder,
                0f,
                TOP_BAR_HEIGHT - TOP_BAR_BORDER_HEIGHT,
                screenWidth,
  ...
  private void drawBarValue(
      SpriteBatch batch, String value, float barX, float barY, float width, float height) {
    GlyphLayout layout = new GlyphLayout(statLabelFont, value);
    statLabelFont.draw(
        batch, value, barX + (width - layout.width) / 2f, barY + (height - layout.height) / 2f);
  }
  ...
  private final class HudStatsPanel extends AbstractGuiElement implements GuiResizable {
    ...
    private void scaleChild(GuiResizable child, float scaleX, float scaleY) {
      child.setPosition(x + (child.getX() - x) * scaleX, y + (child.getY() - y) * scaleY);
  ...
  private void renderCombatModeIndicator(SpriteBatch batch) {
    ...
    batch.draw(
        combatModeIcon.getTexture(),
        x,
        y,
        iconW,
        iconH,
        combatModeIcon.getRegionX(),
        combatModeIcon.getRegionY(),
        combatModeIcon.getRegionWidth(),
        combatModeIcon.getRegionHeight(),
        false,
        true);
  }
  ...
  private void renderBuffDurationBar(SpriteBatch batch, Player.ActiveBuff buff, float x, float y) {
    ...
    float percent =
        buff.getDurationMillis() <= 0L
            ? 0f
            : Math.min(1f, remainingMillis / (float) buff.getDurationMillis());
    float barX = x + 3.55f;
    float barY = y + 2f;
    float barW = 2;
  ...
  private void renderQuickBar(SpriteBatch batch) {
    ...
    for (int i = 0; i < QUICK_SLOT_COUNT; i++) {
      ...
      TextureRegion icon =
          itemName == null ? resolveSpellIcon(spellName) : resolveItemIcon(itemName);
      if (icon != null) {
        float iconScale =
            Math.min(
                1f,
  ...
  private void renderExperienceBar(SpriteBatch batch) {
    ...
    if (com.perso.T4C.config.GamePreferencesStore.get().isXpBarText()) {
      drawBarValue(
          batch,
          "XP  " + Math.round(ratio * 100f) + "%",
          lastXpGuiX,
          lastXpGuiY - 3f * layout.scale,
          lastXpGuiWidth,
  ...
  private void renderDraggedQuickSlotIcon(SpriteBatch batch) {
    ...
    batch.draw(
        icon.getTexture(),
        draggedQuickSlotX - iconW * 0.5f,
        draggedQuickSlotY - iconH * 0.5f,
        iconW,
        iconH,
        icon.getRegionX(),
        icon.getRegionY(),
  ...
  public boolean isHudHit(int screenX, int screenY) {
    ...
    if (screenX >= barX
        && screenX <= barX + CHAT_BAR_WIDTH * scale
        && screenY >= barY
    ...
    public void render(SpriteBatch batch) {
      if (goldPanelTexture != null) {
        batch.draw(
            ...
            0,
            0,
            goldPanelTexture.getWidth(),
            goldPanelTexture.getHeight(),
            false,
            true);
      }
      ...
      statLabelFont.draw(
          batch, value, x + (textWidth - layout.width) * 0.5f, y + (height - layout.height) * 0.5f);
      GuiBoxedItem.drawDebugBorder(batch, x, y, width, height);
    }
  ...
  private QuickBarLayout quickBarLayout() {
    ...
    return new QuickBarLayout(
        barX + QUICK_SLOT_X * scale, barY + QUICK_SLOT_Y * scale, QUICK_SLOT_SIZE * scale, scale);
  }
  ...
  private static final class QuickSlotBox extends AbstractGuiElement implements GuiResizable {
    ...
    @Override
    public QuickSlotBox setSize(float width, float height) {
      this.width = Math.max(8f, width);
      this.height = Math.max(8f, height);
      return this;
    }
  ...
  private float chatBarButtonX(QuickBarLayout layout, ChatBarButtonItem item) {
    return layout.x + (item.baseX - QUICK_SLOT_X + item.offsetX) * layout.scale;
  }

  private float chatBarButtonY(QuickBarLayout layout, ChatBarButtonItem item) {
    return layout.y + (item.baseY - QUICK_SLOT_Y + item.offsetY) * layout.scale;
  }
  ...
  public boolean chatBarButtonsTouchDragged(float screenX, float screenY) {
    ...
    item.offsetX =
        (screenX - item.grabOffsetX - layout.x) / layout.scale + QUICK_SLOT_X - item.baseX;
    item.offsetY =
        (screenY - item.grabOffsetY - layout.y) / layout.scale + QUICK_SLOT_Y - item.baseY;
    return true;
  }
  ...
  private void loadChatBarButtons() throws GameException {
    ...
    addChatBarButton(
        "Character", 664f, BACKPACK_BUTTON_Y, "GUI_ChatBtnCharCheet", () -> characterAction.run());
    addChatBarButton(
        "Backpack",
        BACKPACK_BUTTON_X,
        BACKPACK_BUTTON_Y,
        "GUI_ChatBtnBackPack",
        () -> backpackAction.run());
    addChatBarButton("Spell book", 750f, "GUI_ChatBtnSpell", () -> spellBookAction.run());
    addChatBarButton("Quest journal", 793f, "GUI_ChatBtnQuest", () -> questAction.run());
    addChatBarButton("World map", 836f, "GUI_ChatBtnMap", () -> mapAction.run());
    addChatBarButton(
        "Options", 879f, BACKPACK_BUTTON_Y, "GUI_ChatBtnOption", () -> optionsAction.run());
  }
  ...
  private void addChatBarButton(
      String logName, float x, float y, String spriteBase, Runnable action) throws GameException {
    ...
    if (normal != null && hover != null && pressed != null) {
      chatBarButtons.add(
          new ChatBarButtonItem(
              logName, x, y, new GuiButton(normal, hover, pressed, 0f, 0f, action)));
    }
  }
  ...
  private TextureRegion resolveItemIcon(String itemName) {
    ...
    return item == null
        ? null
        : spriteLoader.getRegionFromSpriteName(item.getAppearanceInventory());
  }
  ...
  public void updateQuickSlotGuiItemDrag(float screenX, float screenY) {
    ...
    quickSlotGuiOffsetX[index] = (targetX - layout.x) / layout.scale - QUICK_SLOT_OFFSET_X[index];
  ...
  private void renderCooldownText(
      SpriteBatch batch, float slotX, float slotY, float size, SpellData spell) {
    if (spell == null) {
      return;
    }
  ...
  private String buildBuffTooltipText(Player.ActiveBuff buff) {
    ...
    return description
        + ". "
        + I18n.key("tooltip.time_remaining")
        + ": "
        + formatBuffRemaining(buff);
  }
