...
public class FloatingDamage {
  ...
  public FloatingDamage() {
    fontNormal =
        FontManager.getInstance()
            .getT4CBeaulieuFont(16, Color.WHITE, 1.2f, Color.BLACK, 0, 0, Color.CLEAR);
    fontCrit =
        FontManager.getInstance()
            .getT4CBeaulieuFont(22, Color.WHITE, 1.5f, Color.BLACK, 0, 0, Color.CLEAR);
  }
  ...
  public void spawn(int amount, float worldX, float worldY, Type type, boolean crit) {
    String prefix =
        type == Type.PLAYER_RECEIVED ? "-" : type == Type.HEAL || type == Type.MANA ? "+" : "";
