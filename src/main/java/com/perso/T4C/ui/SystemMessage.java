...
public class SystemMessage {
    ...
    public SystemMessage() {
        this.font = FontManager.getInstance().getT4CBeaulieuFont(
                FONT_SIZE,
                MESSAGE_COLOR,
                OUTLINE_WIDTH,
                OUTLINE_COLOR,
    ...
    /**
     * Shows a line that is being edited right now (NPC talk input), replacing whatever
     * is on screen instead of queueing behind it. A keystroke must be visible on the very
     * next frame: going through {@link #show(String)} would stack one 3s toast per
     * character and the typed text would appear letter by letter, seconds late.
     */
    public void showLive(String message) {
        ...
        queue.clear();
        if (message.isEmpty()) {
            // Cancelling input clears the line at once rather than parking an empty toast.
            currentMessage = null;
            messageTimer = 0f;
            return;
        }
    ...
    public void render(SpriteBatch batch) {
        ...
        GlyphLayout layout = new GlyphLayout(font, currentMessage, font.getColor(),
                wrapWidth, Align.center, true);
