...
public class EditorDropdownList<T> extends EditorControl {
  ...
  public EditorDropdownList<T> labelProvider(Function<T, String> labelProvider) {
    this.labelProvider =
        labelProvider == null ? value -> value == null ? "" : value.toString() : labelProvider;
    return this;
  }
  ...
  public void renderDropdown(SpriteBatch batch, ShapeRenderer shapeRenderer, BitmapFont font) {
    ...
    for (int i = scrollOffset; i < end; i++) {
      if (i == selectedIndex) {
        ...
        shapeRenderer.rect(
            bounds.x + 2f, rowY + 1f, bounds.width - scrollbarReserve() - 4f, rowHeight - 2f);
        shapeRenderer.setColor(EditorTheme.SURFACE);
      }
    }
  ...
  private void layoutScrollBar() {
    ...
    float thumbHeight =
        items.isEmpty()
            ? scrollTrackBounds.height
            : Math.max(
                24f, scrollTrackBounds.height * (rows / (float) Math.max(rows, items.size())));
  ...
  public boolean handleClick(int screenX, int screenY, int button) {
    ...
    if (items.size() > displayedRows() && scrollTrackBounds.contains(screenX, screenY)) {
      ...
      if (maxOffset > 0) {
        float ratio =
            (scrollTrackBounds.y + scrollTrackBounds.height - screenY) / scrollTrackBounds.height;
