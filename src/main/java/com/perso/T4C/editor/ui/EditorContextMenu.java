...
public class EditorContextMenu extends EditorComponent {
  ...
  public void openAt(int screenX, int screenY, float width, float menuBarHeight) {
    ...
    float y =
        Math.max(8f, Math.min(uiY - height, Gdx.graphics.getHeight() - menuBarHeight - height));
  ...
  private void layoutSubmenu(Item parent) {
    ...
    submenuY =
        Math.max(
            8f, Math.min(rowTop - submenuHeight, Gdx.graphics.getHeight() - submenuHeight - 8f));
  }
  ...
  private int submenuRowAt(int screenX, int uiY) {
    if (openSubmenuIndex < 0
        || screenX < submenuX
        || screenX > submenuX + submenuWidth
        || uiY < submenuY
        || uiY > submenuY + submenuHeight) {
      return -1;
    }
