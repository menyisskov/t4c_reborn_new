...
/**
 * Handles player movement state (current facing angle, moving flag and horizontal flip)
 * and performs position updates with clamping to map bounds and collision detection.
 */
@Getter
 ...
 */
    ...
    /** Stops both keyboard animation state and any pending click-to-move path. */
    public void stop() {
        moving = false;
        path.clear();
        clearActiveGridStep();
    ...
    /**
     * Repairs a saved position produced by the former continuous collision
     * code. Such a save can have a free logical anchor while its configured
     * safety footprint is already inside a wall.
     *
     * @return {@code true} when the player was moved to the nearest clear grid
     *         anchor.
     */
    public boolean recoverLoadedCollisionPosition(Player player) {
        ...
        if (bestX == Integer.MIN_VALUE) {
            log.warn("Could not recover loaded player collision position at tile=({},{})",
                    originX, originY);
            return false;
        }
        ...
        clearActiveGridStep();
        moving = false;
        log.info("Recovered loaded player collision position from tile=({},{}) to tile=({},{})",
                originX, originY, bestX, bestY);
        return true;
    }

    /**
     * Update player position using input deltas and elapsed time.
     * Also sets movement flags and facing direction.
     *
     * Logical T4C movement is cell based: the complete destination is accepted
     * or rejected first, then the sprite is interpolated toward that reserved
     * cell. This prevents a held key from advancing partway into a rejected
     * destination and visibly recoiling on a later frame.
     */
    public void move(Player player, float dx, float dy, float delta) {
        ...
        float travelBudget = GameConstants.PLAYER_SPEED
        ...
        while (true) {
            ...
            if (activeGridStep) {
                ...
                advancedThisUpdate |= Math.abs(advancedPos.getX() - beforeX) >= POSITION_EPSILON
            ...
            if (!directInput) {
                ...
                requestedDy = Math.signum(target.y * GRID_H - pos.getY())
                        * ((float) GRID_H / (float) GRID_W);
            }
            ...
            /*
             * An old build or an authoritative correction can leave a
             * fractional position while no grid step is active. Keep that
             * exact accepted position and reserve the next anchor ahead of it;
             * snapping back first caused a visible, almost one-cell recoil.
             */
            float targetX = reservationTarget(pos.getX(), GRID_W, inputX);
            float targetY = reservationTarget(pos.getY(), GRID_H, inputY);
            CollisionManager collisionManager = CollisionManager.getInstance();
            boolean checkCollisions = player.isPlayerCollisionsEnabled()
                    && collisionManager.isInitialized();
            boolean primaryAccepted = isWithinMapBounds(player, targetX, targetY)
                    && (!checkCollisions || canReservePlayerStep(
                            collisionManager,
                            pos.getX(),
                            pos.getY(),
                            targetX,
            ...
            if (primaryAccepted) {
                reserveGridStep(pos, targetX, targetY, inputX, inputY);
                    ...
                    && tryReserveAdjacentDirection(
                            player, collisionManager, pos, inputX, inputY)) {
                // One of the two adjacent T4C directions was reserved.
            } else {
    ...
    /**
     * Reproduces {@code Character::MoveUnit}: after a rejected direction the
     * server tries {@code where - 1}, then {@code where + 1}, wrapping around
     * the eight T4C directions. This also applies to cardinal input: north can
     * fall back to northwest/northeast and east to northeast/southeast.
     *
     * <p>The original server stops there. When both diagonals stay inside the
     * blocking row or column — walking straight into a wall that spans the
     * whole leading edge — that fallback yields nothing and the puppet halts.
     * A third pass therefore tries the two perpendicular directions
     * ({@code where ± 2}) so the player slides along the obstacle edge instead
     * of stopping against it. Those steps keep no component of the rejected
     * axis, so they never push into the wall.
     */
    private boolean tryReserveAdjacentDirection(
        ...
        // Closest directions first, each side before widening the angle.
        for (int offset = 1; offset <= 2; offset++) {
            for (int sign : new int[]{-1, 1}) {
                int[] move = fromT4cDirection(
                        wrapT4cDirection(requestedDirection + sign * offset));
                if (tryReserveSlideStep(
                        player, collisionManager, pos, move[0], move[1], inputX, inputY)) {
                    return true;
                }
            }
        }
    ...
    /**
     * Rejects one footprint probe, expressed as an offset from the step's
    ...
    /**
     * Walks the puppet's leading footprint for a step and lets {@code probe}
     * decide whether each cell rejects it.
     *
     * <p>The horizontal, vertical and diagonal probes are tested separately on
     * purpose: on staircase walls a diagonal corner probe alone can jump across
     * one blocked stair.
     */
    ...
    /**
     * Checks the logical destination and the puppet's leading footprint. A
     * purely diagonal step may keep an upper contact it already had, which is
     * what lets the puppet follow a wall it is already brushing against.
     */
    ...
    /**
     * A C++ adjacent-direction fallback may move tangentially while its
     * footprint already touches a wall. Allow that contact to remain, but
     * reject any collision newly introduced by the slide.
     */
    ...
    /**
     * Converts a signed grid vector to the original clockwise direction ring:
     * N, NE, E, SE, S, SW, W, NW.
     */
    ...
    private int[] fromT4cDirection(int direction) {
        return switch (direction) {
            ...
            default -> throw new IllegalArgumentException(
                    "T4C movement direction must be between 1 and 8");
        };
    }
    ...
    private float continueActiveGridStep(Player player, Coordinates pos, float travelBudget) {
        ...
        float toX = moveToward(
                pos.getX(),
                activeStepTargetX,
                Math.abs(activeStepMoveX) * pixelStep);
        float toY = moveToward(
                pos.getY(),
                activeStepTargetY,
                Math.abs(activeStepMoveY) * pixelStep * GRID_H / GRID_W);

        boolean arrived = Math.abs(toX - activeStepTargetX) < 0.001f
                && Math.abs(toY - activeStepTargetY) < 0.001f;
        if (arrived) {
    ...
    /**
     * Destination anchor for one reserved step along a single axis. A zero
     * direction keeps the current coordinate, so an axis the input does not
     * touch is never re-snapped.
     */
    ...
    private void maybeLogBlockedMove(CollisionManager collisionManager, float worldX, float worldY) {
        ...
        log.info("Move blocked at world=({},{}) tile=({},{}), collisionValue={}, collisionMap={}x{}",
                (int) worldX, (int) worldY, gridX, gridY, value,
                collisionManager.getCollisionWidth(), collisionManager.getCollisionHeight());
    }

    /**
     * Determine currentAngle and flipX from input direction components.
     */
    private void computeAngleFlip(float dx, float worldDy) {
        if (dx == 0 && worldDy > 0) { currentAngle = "180"; flipX = false; }
        else if (dx == 0 && worldDy < 0) { currentAngle = "000"; flipX = false; }
        else if (dx < 0 && worldDy == 0) { currentAngle = "090"; flipX = false; }
        else if (dx > 0 && worldDy == 0) { currentAngle = "090"; flipX = true; }
