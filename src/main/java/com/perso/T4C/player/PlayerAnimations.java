...
public class PlayerAnimations extends EntityAnimationsBase {
    ...
    if (attacking) {
      ...
      if (attackTimer > FRAME_DURATION) {
        ...
        Map<String, Map<String, List<TextureRegion>>> activeAttack =
            bowAttack ? bowAttackAnimations : attackAnimations;
  ...
  public void renderComposite(SpriteBatch batch, Vector2 pos, PlayerMovement movement) {
    renderComposite(
        batch, pos, movement.getCurrentAngle(), movement.isFlipX(), movement.isMoving());
  }

  public boolean isMouseOver(float worldX, float worldY, Vector2 pos, PlayerMovement movement) {
    return isMouseOver(
        worldX, worldY, pos, movement.getCurrentAngle(), movement.isFlipX(), movement.isMoving());
  }

  public Rectangle getRenderBounds(Vector2 pos, PlayerMovement movement, Rectangle out) {
    return getRenderBounds(
        pos, movement.getCurrentAngle(), movement.isFlipX(), movement.isMoving(), out);
  }
  ...
  public void renderComposite(
    ...
    batch.draw(
        outlineRegion, renderBounds.x - margin, renderBounds.y - margin + height, width, -height);
  }

  public boolean isMouseOver(
      float worldX, float worldY, Vector2 pos, String angle, boolean flipX, boolean moving) {
    collectDrawParts(pos, angle, flipX, moving, drawParts, renderBounds);
  ...
  public float getDepthY(Vector2 pos, String angle, boolean flipX, boolean moving) {
    ...
    BodyPart[] order =
        PuppetBodyOrder.getBodyPartsOrder(
            ...
            bowEquipped,
            attackPoseVisible,
            attackPoseVisible ? attackFrame : frameIndex);
  ...
  private void renderOutlinedComposite(
      SpriteBatch batch,
      ShaderProgram outlineShader,
      float outlineR,
      float outlineG,
  ...
  private void renderPartsToCompositeBuffer(SpriteBatch batch, int margin, int width, int height) {
    ...
    outlineBuffer.end();
    Gdx.gl.glViewport(
        viewportBuffer.get(0), viewportBuffer.get(1), viewportBuffer.get(2), viewportBuffer.get(3));
    batch.begin();
  }
  ...
  private void ensureOutlineBuffer(int width, int height) {
    ...
    if (outlineBuffer != null
        && outlineBuffer.getWidth() == width
        && outlineBuffer.getHeight() == height) {
      return;
    }
    if (outlineBuffer != null) {
  ...
  private void loadAnimationsForBase(
      String base,
      String prefix,
      Map<String, List<TextureRegion>> target,
      boolean allowSingleFrame,
      SpriteLoader loader)
  ...
  private static final class ResolvedFrames {
    ...
    private ResolvedFrames(
        String base, String prefix, List<TextureRegion> frames, boolean attackSequence) {
      this.base = base;
      this.prefix = prefix;
