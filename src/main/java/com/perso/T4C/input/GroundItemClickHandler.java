...
/**
 * Handles left-clicks to pick up ground items (monster loot) within interaction range.
 * Modeled on {@link ObjectClickHandler}.
 */
public class GroundItemClickHandler extends InputAdapter {
    ...
    public GroundItemClickHandler(OrthographicCamera camera, GroundItemManager groundItemManager,
                                  Player player, SystemMessage systemMessage) {
        this.camera = camera;
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        int tileDistance = Math.max(Math.abs(clickTileX - playerTileX), Math.abs(clickTileY - playerTileY));

        GroundItemManager.PickupResult result = groundItemManager.pickUpAt(
                worldCoordsTemp.x, worldCoordsTemp.y, player, tileDistance, OBJECT_MAX_INTERACTION_DISTANCE);
