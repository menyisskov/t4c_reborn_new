...
/**
 * Handles clicks on WDA objects. When an object with clickAnimate=true is clicked,
 * triggers its animation sequence if the player is within 3 tiles distance.
 */
public class ObjectClickHandler extends InputAdapter {
    ...
    public ObjectClickHandler(ObjectRenderer objectRenderer, List<ObjectPos> objectPositions, MapRenderer mapRenderer, OrthographicCamera camera, Player player, SystemMessage systemMessage, GroundItemManager groundItemManager) {
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        if (button == com.badlogic.gdx.Input.Buttons.LEFT) {
            ...
            boolean objectClicked = objectRenderer.handleClick(
                worldCoords.x,
                worldCoords.y,
                objectPositions,
                mapRenderer.getObjectMappings(),
