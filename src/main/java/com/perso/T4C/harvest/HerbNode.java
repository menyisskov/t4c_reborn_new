...
/** One visible, interactive herb resource in the world. */
public final class HerbNode {
    ...
    public enum State { AVAILABLE, HARVESTING, HARVESTED }
    ...
    public float getDepthY() { return position.y / GRID_H; }

    public void showName() { nameDisplayUntil = System.currentTimeMillis() + Nameable.NAME_DISPLAY_MS; }

    public boolean isNameVisible() { return System.currentTimeMillis() < nameDisplayUntil; }
    ...
    public void render(SpriteBatch batch, ShaderProgram outlineShader) {
        ...
        batch.draw(r.getTexture(), position.x - w * .5f, position.y - h,
                w, h, r.getRegionX(), r.getRegionY(), r.getRegionWidth(), r.getRegionHeight(), false, true);
        ...
        if (isNameVisible()) {
            ...
            NameRenderer.renderName(batch, label, position.x - 15f,
                    position.y - h + NAME_RENDERER_VERTICAL_OFFSET - NAME_LABEL_ABOVE_HERB, w, h);
        }
    }
