...
public final class InventoryService {
  ...
  public static long currentWeight(Player player) {
    ...
    for (Map.Entry<BodyPart, String> entry : player.getEquippedItems().entrySet()) {
      ...
      if (definition == null || isMirroredSecondarySlot(player, slot, entry.getValue(), definition))
        continue;
  ...
  public static Result add(Player player, String itemKey, int remainingCharges) {
    ...
    if (definition.isUnique() && count(player, canonicalKey) > 0)
      return Result.failure(Failure.UNIQUE_ITEM, canonicalKey);
  ...
  public static Result validateEquip(Player player, BodyPart requestedSlot, String itemKey) {
    ItemDefinition definition = ItemRegistry.findByKey(itemKey);
    if (player == null || definition == null) return Result.failure(Failure.UNKNOWN_ITEM, itemKey);
    if (!player.getInventory().contains(itemKey))
      return Result.failure(Failure.ITEM_NOT_OWNED, itemKey);
    if (!slotMatches(requestedSlot, definition.getBodyPart()))
      return Result.failure(Failure.WRONG_SLOT, itemKey);
    ...
    if (!meetsRequirements(player, definition))
      return Result.failure(Failure.REQUIREMENTS_NOT_MET, itemKey);
    return Result.success(itemKey);
  }
  ...
  public static Result unequip(Player player, BodyPart slot) {
    ...
    ItemDefinition definition = ItemRegistry.findByKey(itemKey);
    if (definition != null
        && definition.getSecondaryBodyPart() == slot
  ...
  public static Result useCharge(Player player, String itemKey) {
    ...
    int remaining =
        player
            .getItemCharges()
            .getOrDefault(itemKey, definition.getNbCharges() * Math.max(1, count(player, itemKey)));
  ...
  public static int chargesForNextInstance(Player player, String itemKey) {
    ...
    if (player == null
        || definition == null
        || definition.isUnlimitedUse()
        || definition.getNbCharges() <= 0) {
      return -1;
    }
    int total =
        player
            .getItemCharges()
            .getOrDefault(itemKey, definition.getNbCharges() * count(player, itemKey));
  ...
  public static void removeExtractedCharges(Player player, String itemKey, int extractedCharges) {
    ...
    int remaining =
        Math.max(0, player.getItemCharges().getOrDefault(itemKey, 0) - extractedCharges);
    if (remaining > 0) player.getItemCharges().put(itemKey, remaining);
    else player.getItemCharges().remove(itemKey);
  }
  ...
  private static boolean isMirroredSecondarySlot(
      Player player, BodyPart slot, String key, ItemDefinition definition) {
