...
/** Executes scrolls, potions and other spell-bearing items, including charges. */
public final class ItemUseService {
    public enum Failure { NONE, NOT_OWNED, NO_EFFECT, NO_HEALING_NEEDED, NO_MANA_NEEDED, SPELL_FAILED, CHANCE_FAILED }

    public record Result(boolean success, Failure failure, String spellName) {
        private static Result failure(Failure failure) { return new Result(false, failure, null); }
        private static Result success(String spell) { return new Result(true, Failure.NONE, spell); }
    }
    ...
    public static Result useOnSelf(Player player, String itemKey, RandomGenerator random) {
        ...
        if (healthDelta > 0 && player.getCurrentHp() >= player.getMaxHp()) return Result.failure(Failure.NO_HEALING_NEEDED);
        if (manaDelta > 0 && player.getMana() >= player.getMaxMana()) return Result.failure(Failure.NO_MANA_NEEDED);
        SpellCastingService.Result cast = SpellCastingService.beginItemUse(new SpellCastingService.Request(
        ...
        if (manaDelta != 0) {
            ...
            // Report what was actually gained: the cap may absorb part of the potion.
        ...
        if (!originalBuffs.isEmpty()) {
            player.applyBuff(spell.getName(), spell.getDescription(), spell.getIconId(),
                    manager.resolveDurationSeconds(spell, player), false, originalBuffs);
        } else {
    ...
    private static void applyBuff(Player player, SpellData spell) {
        ...
        if (buff == null) return;
        player.applyBuff(spell.getName(), spell.getDescription(), spell.getIconId(),
                buff.getDurationSeconds(), Boolean.TRUE.equals(buff.getUnlimited()), buff.getEffects());
    }
}
