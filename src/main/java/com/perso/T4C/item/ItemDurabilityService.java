...
public final class ItemDurabilityService {
  ...
  private static final Set<BodyPart> ARMOR_SLOTS =
      Set.of(
          BodyPart.HEAD,
          BodyPart.BODY,
          BodyPart.LEGS,
  ...
  public static boolean isRepairable(ItemDefinition item) {
    ...
    return item.getBodyPart() == BodyPart.WEAPON
        || item.getBodyPart() == BodyPart.WEAPON2
  ...
  public static void synchronize(Player player) {
    ...
    while (durability.size() > player.getInventory().size())
      durability.remove(durability.size() - 1);
  ...
  public static double inventory(Player player, int index) {
    ...
    return player == null || index < 0 || index >= player.getInventoryDurability().size()
        ? MAX
        : player.getInventoryDurability().get(index);
  }
  ...
  public static long repairCost(ItemDefinition item, double durability) {
    ...
    return Math.max(
        1L,
        (long) Math.ceil(Math.max(0L, item.getPrice()) * (MAX - clamp(durability)) / 100d * .20d));
  }
  ...
  public static long repairAllCost(Player player) {
    ...
    for (int i = 0; i < player.getInventory().size(); i++)
      total +=
          repairCost(ItemRegistry.findByKey(player.getInventory().get(i)), inventory(player, i));
    ...
    for (BodyPart slot : player.getEquippedItems().keySet()) {
      ...
      if (seen.add(primary))
        total +=
            repairCost(
                ItemRegistry.findByKey(player.getEquippedItems().get(primary)),
                equipped(player, primary));
    }
    return total;
  }
