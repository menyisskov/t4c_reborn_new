...
public final class EquipmentBonusRules {
  ...
  public static int bonus(Player player, int statId) {
    ...
    for (Map.Entry<BodyPart, String> equipped : player.getEquippedItems().entrySet()) {
      ...
      if (item == null
          || ItemDurabilityService.isBroken(player, equipped.getKey())
          || isMirrored(player, equipped.getKey(), equipped.getValue(), item)) continue;
      for (ItemDefinition.ItemBoost boost : item.getBoosts()) {
        if (player.getIntelligence() < boost.getMinInt() || player.getWisdom() < boost.getMinWis())
          continue;
        active.put(
            boost.getBoostId(), new ActiveBoost(boost, evaluate(player, boost.getExpression())));
      }
    }
    long total = 0;
    for (ActiveBoost value : active.values())
      if (value.definition.getStatId() == statId) total += value.amount;
    ...
    String formula =
        expression == null
            ...
            : expression
                ...
                .replaceAll(
                    "true_skill\\(35\\)", Integer.toString(player.getSkillLevel("archery")));
  ...
  private static boolean isMirrored(Player player, BodyPart slot, String key, ItemDefinition item) {
    return item.getSecondaryBodyPart() == slot
        && key.equals(player.getEquippedItems().get(item.getBodyPart()));
  }
