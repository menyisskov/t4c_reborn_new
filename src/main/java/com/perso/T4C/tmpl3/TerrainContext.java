...
public final class TerrainContext {
  ...
  private TerrainContext(
      int x,
      int y,
      Map<Direction, TerrainSample> samples,
      String familyA,
      String familyB,
      String terrainA,
      String terrainB) {
  ...
  public static TerrainContext analyze(
      int x, int y, int width, int height, TerrainResolver resolver) {
  ...
  private static String pickFamily(Map<String, TerrainScore> scores, String excluded) {
    return scores.values().stream()
        ...
        .max(
            Comparator.comparingInt((TerrainScore score) -> score.count)
