...
public final class Tmpl3DebugExporter {
    ...
    public File exportComparison(File outputDir, Tmpl3Point position, TerrainContext context, MaskColor[][] expected,
            List<Tmpl3Candidate> candidates, Tmpl3Candidate chosen) {
        ...
        try {
            ...
            log.info("Tmpl3 debug ({}, {}): families {}|{}, chosen {}, candidates {}",
                    position.x(), position.y(), context.familyA(), context.familyB(),
                    chosen != null ? chosen.id() : null,
                    candidates.stream()
                            .limit(5)
                            .map(c -> c.id() + "=" + c.finalScore())
                            .toList());
            return output;
        } finally {
            image.dispose();
