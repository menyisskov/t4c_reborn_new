...
public final class Tmpl3Matcher {
    ...
    public List<Tmpl3Candidate> findBestCandidates(MaskColor[][] expected, int limit) {
        return masks.stream()
                ...
                .sorted(Comparator.comparingDouble(Tmpl3Candidate::terrainScore).thenComparingInt(Tmpl3Candidate::id))
    ...
    private static double pixelWeight(int x, int y) {
        ...
        boolean diagonal = Math.abs((x / 31d) - (y / 15d)) < 0.10d
                || Math.abs((x / 31d) - (1d - y / 15d)) < 0.10d;
        if (diagonal) {
            return 4d;
        }
