...
/**
 * Generates .colbin collision maps from editable collision rules.
 */
public final class CollisionGenerationService {
    ...
    public static Result regenerate(File mapFile, Map<String, SpriteMeta> spriteMetaByLowerName) throws IOException, GameException {
        if (mapFile == null || !mapFile.exists()) {
            throw new IOException("Map file not found: " + (mapFile == null ? "null" : mapFile.getPath()));
        }
        CollisionRules rules = new File(Paths.COLLISION_RULES_BIN).exists()
        ...
        try (MapReader reader = new MapReader(mapFile, true)) {
            ...
            count += generateForObjects(data, reader.getWidth(), reader.getHeight(), rules, spriteMetaByLowerName);
    ...
    private static int generateForMap(MapReader reader, byte[] data, CollisionRules rules, Map<String, SpriteMeta> metaByName) {
        ...
        for (int y = 0; y < reader.getHeight(); y++) {
            for (int x = 0; x < reader.getWidth(); x++) {
                ...
                int originX = x;
                int originY = y;
                SpriteMeta meta = metaByName != null
                        ? metaByName.get(resolved.name.toLowerCase(Locale.ROOT))
                        : null;
                if (meta != null) {
                    originX += Math.round(reader.getOffsetXFast(x, y) / GameConstants.GRID_W);
                    originY += Math.round((reader.getOffsetYFast(x, y) + meta.height - GameConstants.GRID_H)
                ...
                count += applyRulesForSpriteNameAt(data, reader.getWidth(), reader.getHeight(), originX, originY,
                        resolved.name, resolved.mirror, rules, ignoredSprites, exactRules);
            }
        }
        return count;
    }

    private static int generateForObjects(byte[] data, int width, int height, CollisionRules rules, Map<String, SpriteMeta> metaByName) {
        ...
        try {
            for (ObjectPos pos : ObjectPositionBinaryIO.read(positionsFile)) {
                ...
                count += applyRulesForSpriteNameAt(data, width, height, x, y, objectName, false, rules, ignoredSprites, exactRules);
                count += applyRulesForSpriteNameAt(data, width, height, x, y, mappedSpriteName, false, rules, ignoredSprites, exactRules);
            }
        } catch (Exception ignored) {
        }
        return count;
    ...
    private static int applyRulesForSpriteNameAt(byte[] data, int width, int height, int x, int y, String sourceName, boolean mirrored,
    ...
    private static int applyCollisionRule(byte[] data, int width, int height, int originX, int originY, CollisionRule rule, boolean mirrored) {
        if (rule == null) {
            return 0;
        }
    ...
    private static int setCollisionIfEmpty(byte[] data, int width, int height, int x, int y, int value) {
    ...
    private static void writeCollisionData(File file, int width, int height, byte[] data) throws IOException {
    ...
    private static Resolved resolveSprite(String rawName, Map<String, SpriteMeta> metaByName) {
        ...
        if (trimmed.endsWith("M")) {
            ...
            if (!candidate.isEmpty() && (metaByName == null || metaByName.containsKey(candidate.toLowerCase(Locale.ROOT)))) {
                base = candidate;
                mirror = true;
            }
        }
    ...
    private static int[] getMirrorCollisionOriginShift(String spriteName, Map<String, SpriteMeta> metaByName) {
    ...
    private static boolean isIgnoredSprite(CollisionRules rules, String spriteName, Set<String> exactIgnored) {
    ...
    private static boolean matchesNameRule(CollisionNameRule nameRule, String spriteName) {
        ...
        for (String token : nameRule.contains) {
            ...
            if (normalizedSprite.contains(normalizedToken) || compactSprite.contains(compactSpriteKey(normalizedToken))) {
                return true;
            }
        }
        return false;
    ...
    private static String stripTrailingInstance(String value) {
        ...
        if (isSignedInteger(trimmed, at + 1, comma) && isSignedInteger(trimmed, comma + 1, trimmed.length())) {
    ...
    public record SpriteMeta(String name, int width, int height, int off1X, int off1Y, int off2X, int off2Y) {
    }
