...
/**
 * Shared envelope for the {@code *BinaryIO} catalogue classes: magic header check, version
 * dispatch, entry-count guard, and the read/write loop. Field-level (de)serialization and version
 * compatibility rules stay in each catalogue class, since those vary per format.
 */
public final class BinaryCatalogueIO {
    ...
    public static <T> List<T> read(File file, byte[] magic, String typeLabel, VersionCheck versionCheck,
            DefinitionReader<T> reader) throws IOException, GameException {
    ...
    public static <T> void write(File file, byte[] magic, short version, List<T> definitions,
            DefinitionWriter<T> writer) throws IOException {
