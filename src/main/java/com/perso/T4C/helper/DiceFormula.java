...
/**
 * Parser and evaluator for T4C damage formulas (BoostFormula.cpp equivalent).
 *
 * <p>Supported syntax (from objects.json analysis):
 * <ul>
 *   <li>Basic dice: {@code 1d6}, {@code 2d10+5}, {@code 1d338}</li>
 *   <li>Self variables: {@code self.str}, {@code self.agi}, {@code self.level}, etc.</li>
 *   <li>Arrow damage: {@code arrow_dmg} (passed via Context)</li>
 *   <li>Ternary: {@code if(cond OP val ? true : false)}</li>
 *   <li>Target resistance check: {@code target.r_dark}, {@code target.r_light}</li>
 *   <li>Comparison ops in if: {@code <}, {@code >}, {@code <=}, {@code >=}, {@code ==}, {@code =}, {@code !=}</li>
 *   <li>Arithmetic: {@code +}, {@code -}, {@code *}, {@code /}</li>
 *   <li>Spaces tolerated everywhere (stripped before parsing)</li>
 * </ul>
 */
public final class DiceFormula {
    ...
    public static final class Context {
        ...
        public Context(int str, int end, int agi, int intel, int wil, int wis, int luck, int level,
                       int arrowDmg, int targetResistDark, int targetResistLight) {
            this(str, end, agi, intel, wil, wis, luck, level, arrowDmg, targetResistDark, targetResistLight, 100);
        }

        public Context(int str, int end, int agi, int intel, int wil, int wis, int luck, int level,
                       int arrowDmg, int targetResistDark, int targetResistLight, int selfLight) {
            this(str, end, agi, intel, wil, wis, luck, level, arrowDmg,
                    100, 100, 100, 100, targetResistLight, targetResistDark,
                    100, 100, 100, 100, selfLight, 100);
        }

        public Context(int str, int end, int agi, int intel, int wil, int wis, int luck, int level,
                       int arrowDmg,
                       int targetResistFire, int targetResistEarth, int targetResistAir,
                       int targetResistWater, int targetResistLight, int targetResistDark,
                       int selfFire, int selfEarth, int selfAir, int selfWater, int selfLight, int selfDark) {
            this.str = str; this.end = end; this.agi = agi; this.intel = intel;
            this.wil = wil; this.wis = wis; this.luck = luck; this.level = level;
    ...
    /** Roll a result with no stat context. */
    public int roll() { return roll(Context.ZERO); }

    /** Roll a result substituting variables from {@code ctx}. */
    ...
    /** Evaluates formulas where the sign is meaningful (notably spell health deltas). */
    ...
    /** Minimum possible result (each die = 1). */
    public int min() { return min(Context.ZERO); }
    ...
    /** Maximum possible result (each die = faces). */
    public int max() { return max(Context.ZERO); }
    ...
    private static String prepare(String raw, Context ctx) {
        String s = raw.replaceAll("\\s+", ""); // strip all spaces first
    ...
    private static String substituteVars(String s, Context ctx) {
        // target variables (longer tokens first)
        ...
        // self variables (longer first to avoid partial matches)
    ...
    /**
     * Replace the 'd'/'D' dice operator with '@'.
     * Only replaces 'd' flanked by digit or ')' on left and digit or '(' on right.
     * Mirrors BoostFormula.cpp preprocessing.
     */
    ...
    private static class Parser {
        ...
        double parseExpr() {
            ...
            while (pos < s.length()) {
                ...
                if      (op == '+') { pos++; v += parseTerm(); }
                else if (op == '-') { pos++; v -= parseTerm(); }
                else break;
            }
            return v;
        }
        ...
        double parseTerm() {
            ...
            while (pos < s.length()) {
                char op = s.charAt(pos);
                if (op == '*') { pos++; v *= parseFactor(); }
                else if (op == '/') {
                    pos++;
                    double d = parseFactor();
                    v = d == 0 ? 0 : v / d;
                }
                else if (op == '@') { pos++; v = rollDice((int) v, (int) parseFactor()); }
                else break;
            }
            return v;
        }

        // factor = number | '(' expr ')' | unary '-' | if(...)
        double parseFactor() {
        ...
        /**
         * Parse: if(leftExpr CMP rightExpr ? trueExpr : falseExpr)
         * CMP: <, >, <=, >=, ==, !=, = (T4C uses single '=' as equality)
         */
        double parseIf() {
            pos += 3; // skip "if("
            ...
            char c2 = (pos < s.length() && (s.charAt(pos) == '=' || s.charAt(pos) == '>')) ? s.charAt(pos++) : 0;
            ...
            else /* single '=' = equality */ cond = left == right;
            if (pos < s.length() && s.charAt(pos) == '?') pos++;
    ...
    private static final class MinParser extends Parser {
        MinParser(String s) { super(s); }
        @Override double rollDice(int count, int faces) { return count; }
    }

    private static final class MaxParser extends Parser {
        MaxParser(String s) { super(s); }
        @Override double rollDice(int count, int faces) { return (double) count * faces; }
    }
}
