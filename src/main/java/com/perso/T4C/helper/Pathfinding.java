...
public final class Pathfinding {
    ...
    while (!open.isEmpty()) {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        if (collisionManager != null
            ...
            && isBlocked(nx, ny, collisionManager, effectiveClearance)) {
          continue;
        }
        if (isDiagonal(i) && collisionManager != null && collisionManager.isInitialized()) {
          ...
          if (isBlocked(sideX, sideY, collisionManager, clearanceTiles)
              || isBlocked(sideX2, sideY2, collisionManager, clearanceTiles)) {
            continue;
          }
        }
        ...
        float neighborG =
            gStamp[neighborIdx] == stamp ? gScore[neighborIdx] : Float.POSITIVE_INFINITY;
  ...
  private static boolean isBlocked(
      int x, int y, CollisionManager collisionManager, int clearanceTiles) {
  ...
  private static final long MAX_NANOS = 16_000_000L;
  private static final int TIME_CHECK_MASK = 0x1F;
