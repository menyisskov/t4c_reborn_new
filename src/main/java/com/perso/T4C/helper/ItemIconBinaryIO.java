...
/**
 * Binary serializer for the generic item icon table, the Java port of the C++ client's
 * {@code ItemIcons} table (see {@code Apparence.h} for the {@code __OBJGROUP_*} appearance
 * groups and {@code VisualObjectList.cpp} for the {@code ItemIcons.BindSprite} calls).
 *
 * <p>Each entry binds an appearance id to one of the 24x24 {@code 64kIcon*} category sprites.
 * Several appearances deliberately share a single icon — short and long sword both map to
 * {@code 64kIconSword} — so sprite names are not unique across the table.
 *
 * <p>Layout, wrapped in the shared {@code T4CBIN} deflate container:
 * <pre>
 * "T4CICO"  6 bytes ASCII magic
 * version   int16 LE
 * count     int32 LE
 * count x { appearanceId int32 LE, sprite string }
 * </pre>
 */
