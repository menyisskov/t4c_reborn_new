...
/**
 * Ultra-fast binary map reader with memory-mapped file access.
 * Loads maps in milliseconds instead of seconds.
 *
 * Reads pre-converted .mapbin files for optimal performance.
 */
@Slf4j
public class MapReader implements AutoCloseable {
    ...
    /** Container magic written by {@link BinaryIOUtils#openOutputStream}, ahead of the payload. */
    ...
    // MapReader is used from the render thread: HashMap is faster than ConcurrentHashMap here.
    ...
    /**
     * Opens and memory-maps a binary map file for ultra-fast access (read-only).
     *
     * @param mapFile The map file (can be .json.gz or .mapbin, but .mapbin is expected)
     * @throws GameException if loading fails
     */
    ...
    /**
     * Opens and memory-maps a binary map file for ultra-fast access.
     *
     * @param mapFile The map file (can be .json.gz or .mapbin, but .mapbin is expected)
     * @param readOnly If true, opens in read-only mode; if false, opens in read-write mode
     * @throws GameException if loading fails
     */
    ...
    private MapReader(File mapFile, boolean readOnly, boolean registerSpriteIds, boolean loadTileMetadata) throws GameException {
        ...
        String binaryPath = filePath.endsWith(".mapbin") 
            ? filePath 
            : filePath.replace(".json.gz", ".mapbin");
        
        File binaryFile = new File(binaryPath);
        
        if (!binaryFile.exists()) {
            throw new GameException(
                "Binary map file not found: " + binaryPath + "\n" +
                "Expected file location: " + binaryFile.getAbsolutePath()
            );
        }

        log.info("Loading binary map: {} ({})", binaryFile.getAbsolutePath(), readOnly ? "READ-ONLY" : "READ-WRITE");
    ...
    private MapData loadSpriteNameMap(File binaryFile) throws IOException, GameException {
        ...
        for (int i = 0; i < tileCount; i++) {
            try {
                ...
                log.warn("Reached EOF after {} entries (expected {}). Map file may be truncated.", readEntries, tileCount);
                break;
            }
        }
    ...
    private void loadDecorLayerIfPresent(File mapFile) throws IOException, GameException {
        ...
        if (decorData.width != width || decorData.height != height) {
            throw new GameException("Decor layer dimensions do not match map: " + decorFile.getAbsolutePath());
        }
    ...
    private MapData loadCompactMap(BinaryReader in, int width, int height) throws EOFException, GameException {
        int total = checkedTotalTiles(width, height);
        String[] spriteNames = new String[total];
        float[] scaleX = loadTileMetadata ? new float[total] : null;
        float[] scaleY = loadTileMetadata ? new float[total] : null;
    ...
    private void readScaleOverrides(BinaryReader in, int total, float[] scaleX, float[] scaleY) throws EOFException, GameException {
    ...
    private void readOffsetOverrides(BinaryReader in, int total, float[] offsetX, float[] offsetY) throws EOFException, GameException {
        int count = in.readIntLE();
        if (count < 0) {
    ...
    private void readZOrderOverrides(BinaryReader in, int total, int[] zOrder) throws EOFException, GameException {
        int count = in.readIntLE();
        if (count < 0) {
    ...
    private void writeCompactLayer(File outputFile, String[] names) throws IOException {
        if (!loadTileMetadata || scaleX == null || scaleY == null || offsetX == null || offsetY == null || zOrder == null) {
        ...
        // Written through the T4CBIN Deflate container: a .mapbin is highly repetitive (run-length
        // sprite ids plus mostly-default per-tile metadata) and shrinks to roughly a fifth, which
        // keeps even the largest map well under the 90 MB budget. MapReader#mapFile reads both
        // compressed and raw files, so existing uncompressed maps keep loading unchanged.
        try (DataOutputStream out = new DataOutputStream(
                BinaryIOUtils.openOutputStream(outputFile, 1 << 20))) {
    ...
    private void writeRuns(DataOutputStream out, String[] names, Map<String, Integer> dictionary) throws IOException {
        int total = totalTilesUnchecked(width, height);
        int runStart = 0;
        int previousId = -1;
        for (int i = 0; i <= total; i++) {
    ...
    /**
     * Provides the map bytes, transparently handling the compressed container.
     *
     * <p>A raw {@code T4CMAP} file is memory-mapped, which is what makes loading take milliseconds:
     * the pages are faulted in on demand and never occupy heap. A file wrapped in the {@code T4CBIN}
     * Deflate container cannot be mapped — random access into a Deflate stream is not possible — so
     * it is inflated into a heap buffer instead. That costs roughly 200 ms and the full uncompressed
     * size in heap for the largest map, in exchange for a ~4.5x smaller file on disk.
     */
    ...
    /** True when the file starts with the {@code T4CBIN} container magic rather than {@code T4CMAP}. */
    ...
    /**
     * Fast scale access without bounds checks. Caller must ensure coordinates are valid.
     */
     ...
     * Call this when you're done with the map.
     */
    @Override
    public void close() {
    }
    ...
    /**
     * Hot-path variant used during map loading to minimize CHM/compute/lambda overhead.
     */
 ...
 */
    ...
    private record MapData(int width, int height, String[] spriteNames, float[] scaleX, float[] scaleY,
                           float[] offsetX, float[] offsetY, int[] zOrder) {
    }
    ...
    private static final class BinaryReader {
        ...
        private String readUtf8(int length) throws EOFException {
            ...
            // intern() deduplicates sprite names — many tiles share the same name string,
            // so without intern we create thousands of equal String objects per map load.
