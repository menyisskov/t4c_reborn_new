...
/**
 * Parses sprite names with optional mirror and legacy palette suffixes.
 */
public final class SpriteNameParser {
    private SpriteNameParser() {
    }

    // rawName is intern()'d from MapReader so identity comparison works in the map.
    private static final ConcurrentHashMap<String, ResolvedSprite> PARSE_CACHE = new ConcurrentHashMap<>(4096);

    /**
     * Parse a raw sprite name into base name, mirror flag, and optional metadata.
     *
     * @param rawName    raw sprite name (may include trailing "M" or a legacy palette alias)
     * @param metaByName optional sprite metadata map (lowercase key) used to validate mirror suffix
     * @return ResolvedSprite or null when rawName is empty
     */
    public static ResolvedSprite parse(String rawName, Map<String, SpriteLoader.Sprite> metaByName) {
        if (rawName == null) return null;
        String trimmed = rawName.trim();
        if (trimmed.isEmpty()) return null;
        // metaByName is only used to validate mirror suffix — when null (worldmap path) skip cache
        // to avoid caching potentially wrong results across different callers.
        if (metaByName != null) {
        ...
        String trimmedNoMirror = mirrorSuffix ? trimmed.substring(0, trimmed.length() - 1).trim() : trimmed;
        ...
        if (mirrorSuffix) {
            ...
            if (!candidate.isEmpty()) {
                boolean hasCandidate = metaByName == null || metaByName.containsKey(candidate.toLowerCase(Locale.ROOT));
                boolean hasExactName = metaByName != null && metaByName.containsKey(stripTrailingInstance(base).toLowerCase(Locale.ROOT));
        ...
        // Legacy VSFID mappings distinguish palette variants with a trailing
        // MA/GR/BR/BL token. The packed sprite set contains the corresponding
        // base sprite only. Preserve a real suffixed sprite when one exists,
        // otherwise resolve the legacy alias to that packed base sprite.
    ...
    private static String resolveLegacyPaletteAlias(String value,
    ...
    private static String resolveLegacySpriteAlias(String value,
            Map<String, SpriteLoader.Sprite> metaByName) {
        if (value == null || metaByName == null || value.isBlank()
                || metaByName.containsKey(value.toLowerCase(Locale.ROOT))) {
            return value;
        }
        String alias = switch (value.toLowerCase(Locale.ROOT)) {
            case "cemetery gates 1" -> "Cemetery Gates /^";
            case "cemetery gates 2" -> "Cemetery Gates /";
            case "cemetery gates 3" -> "Cemetery Gates \\v";
            // Gate 4 is the three-way junction; Gate 8 is the lower point.
            case "cemetery gates 4" -> "Cemetery Gates X";
            case "cemetery gates 5" -> "Cemetery Gates -";
            // Legacy Gate 6 is the complete upper junction. The packed ">"
            // asset is only a half-section and leaves a gap between both runs.
            case "cemetery gates 6" -> "Cemetery Gates ^";
            // Legacy Gate 7 is the lateral half-junction (mirrored for 0xC14).
