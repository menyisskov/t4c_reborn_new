...
/**
 * Loads per-sprite modification data (offsets) from a JSON file.
 */
public class ModifSprites {
/**
 * Class representing Offset.
 */
    @NoArgsConstructor
    @AllArgsConstructor
        ...
        try (BufferedReader br = file.reader(256)) {
            ...
            if (data != null) {
                for (Map.Entry<String, Offset> e : data.entrySet()) {
                    // normalize key to lowercase to match lookups
    ...
    /**
     * Return additional offset for the given sprite name (case-insensitive)
     */
