...
public final class QuestDefBinaryIO {
        ...
        version -> {
          if (version != 1 && version != VERSION) {
            throw new GameException(
                "Unsupported quest definition version: " + version + " (expected " + VERSION + ")");
          }
        },
  ...
  private static void writeDefinition(DataOutputStream out, QuestDef definition)
      throws IOException {
    String normalizedId = I18n.normalizedKey(definition.getId());
    writeString(out, definition.getId());
    writeString(
        out, I18n.placeholderForKey("quest." + normalizedId + ".title", definition.getTitle()));
    ...
    writeString(
        out, I18n.placeholderForKey("quest." + normalizedId + ".offer", definition.getOfferText()));
    writeString(
        out,
        I18n.placeholderForKey(
            "quest." + normalizedId + ".completion", definition.getCompletionText()));
    writeString(
        out,
        I18n.placeholderForKey(
            "quest." + normalizedId + ".completed", definition.getCompletedText()));
