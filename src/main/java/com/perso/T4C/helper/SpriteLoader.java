...
public class SpriteLoader {
  ...
  public TextureRegion getEnergyBallPaletteRegion(String name, String palette)
    ...
    try {
      ...
      for (int y = 0; y < pixmap.getHeight(); y++) {
        for (int x = 0; x < pixmap.getWidth(); x++) {
          ...
          int intensity =
              Math.max(
                  (pixel >>> 24) & 0xFF, Math.max((pixel >>> 16) & 0xFF, (pixel >>> 8) & 0xFF));
  ...
  public TextureRegion getMaskedRegionFromSpriteNames(String spriteName, String maskName)
    ...
    String key = spriteName.toLowerCase(Locale.ROOT) + '\0' + maskName.toLowerCase(Locale.ROOT);
    ...
    try {
      ...
      if (mask.getWidth() < sprite.getWidth() || mask.getHeight() < sprite.getHeight()) {
        throw new GameException("Mask is smaller than sprite: " + spriteName + " / " + maskName);
      }
  ...
  public Pixmap createPixmapForRegion(TextureRegion region) {
    ...
    Pixmap result =
        new Pixmap(region.getRegionWidth(), region.getRegionHeight(), fullPixmap.getFormat());
    for (int y = 0; y < region.getRegionHeight(); y++) {
      for (int x = 0; x < region.getRegionWidth(); x++) {
        int pixel = fullPixmap.getPixel(region.getRegionX() + x, region.getRegionY() + y);
