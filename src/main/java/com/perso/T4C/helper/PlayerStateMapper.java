...
public final class PlayerStateMapper {
  ...
  private static List<PlayerStateDto.ActiveBuffState> toActiveBuffStates(Player player) {
    ...
    for (Player.ActiveBuff buff : player.getActiveBuffs()) {
      ...
      state.spellName =
          spell != null ? spell.getKey() : canonicalSpecialBuffKey(buff.getSpellName());
  ...
  private static void applyActiveBuffs(PlayerStateDto state, Player player) {
    ...
    for (PlayerStateDto.ActiveBuffState buff : state.activeBuffs) {
      ...
      boolean unlimited =
          buff.remainingSeconds == Long.MAX_VALUE || buff.totalDurationSeconds == Long.MAX_VALUE;
      if (!unlimited && buff.remainingSeconds <= 0L) {
        continue;
      }
      long restoredDuration =
          buff.totalDurationSeconds > 0L ? buff.totalDurationSeconds : buff.remainingSeconds;
      Integer durationSeconds =
          unlimited || restoredDuration > Integer.MAX_VALUE ? null : (int) restoredDuration;
      boolean seraphAura =
          SeraphAuraService.AURA_NAME.equals(buff.spellName)
