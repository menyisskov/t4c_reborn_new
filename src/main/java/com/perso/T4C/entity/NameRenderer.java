...
public class NameRenderer {
  ...
  private static final Color DIALOG_TEXT_COLOR =
      new Color(230f / 255f, 230f / 255f, 230f / 255f, 1f);
    ...
    try {
      BitmapFont font =
          FontManager.getInstance()
              .getT4CBeaulieuFont(FONT_SIZE, NAME_TEXT_COLOR, 0f, Color.CLEAR, 0, 0, Color.CLEAR);
      GlyphLayout layout = new GlyphLayout(font, name);
      float x = centerX - layout.width / 2f + HORIZONTAL_OFFSET;
      float y = topY - VERTICAL_OFFSET;
      font.setColor(SHADOW_COLOR);
      font.draw(batch, name, x + SHADOW_OFFSET_X, y + SHADOW_OFFSET_Y);
  ...
  public static void renderNameWithKeyword(
      SpriteBatch batch,
      String text,
      String keyword,
      float centerX,
  ...
  public static void renderNameWithKeywords(
      SpriteBatch batch,
      String text,
    ...
    try {
      ...
      for (String line : lines)
        contentWidth = Math.max(contentWidth, new GlyphLayout(font, line).width);
  ...
  private static KeywordHit findFirstWholeWord(String text, List<String> words, int fromIndex) {
    ...
    for (String word : words) {
      ...
      if (candidate >= 0
          && (best < 0
              || candidate < best
              || (candidate == best && word.length() > bestWord.length()))) {
        best = candidate;
        bestWord = word;
      }
    }
  ...
  private static void drawDialogBox(
      SpriteBatch batch, float x, float y, float width, float height) {
    loadDialogSprites();
    if (dialogBack == null) return;
    batch.setColor(Color.WHITE);
    int innerWidth =
        Math.max(0, Math.min(Math.round(width) - DIALOG_BORDER_SIZE, dialogBack.getRegionWidth()));
    int innerHeight =
        Math.max(
            0, Math.min(Math.round(height) - DIALOG_BORDER_SIZE, dialogBack.getRegionHeight()));
    drawCropped(batch, dialogBack, x, y, innerWidth, innerHeight);
    drawCropped(
        batch, dialogBottom, x, y + height - DIALOG_BORDER_SIZE, innerWidth, DIALOG_BORDER_SIZE);
    drawCropped(
        batch, dialogRight, x + width - DIALOG_BORDER_SIZE, y, DIALOG_BORDER_SIZE, innerHeight);
    drawCropped(
        batch,
        dialogBottomRight,
        x + width - DIALOG_BORDER_SIZE,
        y + height - DIALOG_BORDER_SIZE,
        DIALOG_BORDER_SIZE,
        DIALOG_BORDER_SIZE);
  }

  private static void drawCropped(
      SpriteBatch batch, TextureRegion source, float x, float y, int width, int height) {
  ...
  private static void loadDialogSprites() {
    ...
    try {
      ...
      dialogBack =
          loader.getMaskedRegionFromSpriteNames("V3_DialogBoxBack", "V3_DialogBoxBackMask");
      dialogBottom =
          loader.getMaskedRegionFromSpriteNames(
  ...
  public static List<Rectangle> getWordBounds(
      String text, String word, float centerX, float topY, float entityWidth, float entityHeight) {
        ...
        while (index >= 0) {
          ...
          results.add(
              new Rectangle(
                  rx - padding,
                  ry - padding,
                  tokenLayout.width + padding * 2f,
                  lineHeight + padding * 2f));
