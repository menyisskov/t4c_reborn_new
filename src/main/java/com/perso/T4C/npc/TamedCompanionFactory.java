...
public final class TamedCompanionFactory {
    ...
    private static List<CompanionDef.Part> paperdoll(MonsterDef def) {
        ...
        for (int id : ids) for (ItemDefinition item : ItemRegistry.load()) if (item.getNumId() == id) {
    ...
    private static void add(List<CompanionDef.Part> parts, com.perso.T4C.player.BodyPart part, String sprite) {
        if (part != null && sprite != null && !sprite.isBlank()) parts.add(new CompanionDef.Part(part, sprite));
    }
}
