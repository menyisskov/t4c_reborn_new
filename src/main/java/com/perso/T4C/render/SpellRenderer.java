...
public class SpellRenderer {
  ...
  public void triggerImpactSpell(String impactSpell, float worldX, float worldY) {
    ...
    activeImpacts.add(
        new SpellImpact(impactSpell, frames, worldX + impactXOffset, worldY + impactYOffset));
  }

  public void triggerImpactSpell(
      String impactSpell, float worldX, float worldY, String soundImpact) {
  ...
  public boolean launchProjectile(
    ...
    activeProjectiles.add(
        new SpellProjectile(projectileSpell, frames, target, startX, startY, flipX, onImpact));
    return true;
  }
  ...
  public boolean launchProjectile(
    ...
    activeProjectiles.add(
        new SpellProjectile(
            projectileSpell, frames, target, target::getPosition, startX, startY, flipX, onImpact));
    return true;
  }
  ...
  public boolean launchProjectileIfAbsent(
      String projectileSpell,
      BaseMonster target,
      float startX,
      float startY,
  ...
  private void renderChannels(SpriteBatch batch) {
    ...
    for (ChannelEffect channel : activeChannels) {
      ...
      if (position != null && frame != null)
        renderImpactFrame(batch, frame, position.x, position.y);
    }
  }
  ...
  private static final class ChannelEffect {
    ...
    private ChannelEffect(
        ChannelHandle handle, List<ImpactFrame> frames, Supplier<Vector2> positionSupplier) {
      this.handle = handle;
      this.frames = frames;
  ...
  private static final class SpellProjectile {
    ...
    private SpellProjectile(
        ...
        Supplier<Vector2> targetPosition,
        float startX,
        float startY,
        boolean flipX,
        Runnable onImpact) {
