...
/** Runtime equivalent of SpellEffectManager/UnitEffectManager for monster targets. */
public final class SpellEffectManager {
        ...
        for (SpellData.T4cEffect effect : spell.getT4cEffects()) {
            if (effect != null && effect.getEffectType() == 1) {
                ...
                if (formula != null) {
                    ...
                    DiceFormula.Context context = new DiceFormula.Context(base.str, base.end, base.agi, base.intel,
                            base.wil, base.wis, base.luck, base.level, 0,
                            100, 100, 100, 100, 100, 100,
                            elementalPower(caster, "fire"), elementalPower(caster, "earth"),
                            elementalPower(caster, "air"), elementalPower(caster, "water"),
    ...
    /** Converts original AttrBoost effects into the runtime buff model. {@code self} = caster. */
    ...
    /**
     * Same as {@link #resolvePlayerBuffEffects(SpellData, Player)} but with an explicit
     * caster context, for spells cast by something other than the player (e.g. an NPC
     * blessing the player: {@code self} must resolve to the NPC, not the target).
     */
    public List<SpellData.SpellEffect> resolvePlayerBuffEffects(SpellData spell, DiceFormula.Context casterContext) {
        ...
        for (SpellData.T4cEffect effect : spell.getT4cEffects()) {
            ...
            // An instantaneous MANA effect (duration 0, e.g. a mana potion) is a one-shot
            // restore already applied through resolvePlayerManaDelta, not an attribute
            // boost: emitting it here would park a never-expiring icon on the buff bar,
            // since applyBuff maps a zero duration to "unlimited". Timed mana effects
            // (manabane drains and the like) remain real buffs and are kept.
            if ("mana".equals(attribute) && !hasDuration) continue;
            int amount = evaluate(parameter(effect, 3), casterContext);
            // The effect itself is authoritative for buff visibility. Some legacy formulas use
            // variables not yet modelled by DiceFormula; a temporary zero contribution must not
            // make the active spell disappear from the HUD.
            resolved.add(new SpellData.SpellEffect(
    ...
    public PlayerUtility applyPlayerUtilityEffects(SpellData spell, Player caster) {
        ...
        for (SpellData.T4cEffect effect : spell.getT4cEffects()) {
            ...
            if (effect.getEffectType() == 7) {
                ...
                if (targetSpell != null && chance > 0 && ThreadLocalRandom.current().nextInt(101) <= chance
    ...
    private static int evaluate(String formula, Player caster, BaseMonster target, double range) {
        ...
        DiceFormula.Context context = new DiceFormula.Context(
                caster.getEffectiveStrength(), caster.getEffectiveEndurance(), caster.getEffectiveDexterity(),
                caster.getEffectiveIntelligence(), 0, caster.getEffectiveWisdom(), 0, caster.getLevel(), 0,
    ...
    private static DiceFormula.Context casterContext(Player caster) {
        DiceFormula.Context base = SpellCastingService.context(caster);
        return new DiceFormula.Context(base.str, base.end, base.agi, base.intel,
                base.wil, base.wis, base.luck, base.level, 0,
                100, 100, 100, 100, 100, 100,
                elementalPower(caster, "fire"), elementalPower(caster, "earth"),
                elementalPower(caster, "air"), elementalPower(caster, "water"),
    ...
    private static String normalizeBoostAttribute(String attribute) {
        ...
        return switch (attribute.trim().toLowerCase()) {
            ...
            case "air", "fire", "water", "earth", "light", "dark" -> "power:" + attribute.trim().toLowerCase();
            case "mana" -> "mana";
