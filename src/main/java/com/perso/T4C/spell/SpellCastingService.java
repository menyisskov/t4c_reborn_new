...
public final class SpellCastingService {
  ...
  public record Request(
      ...
      boolean pvpTarget,
      boolean knownSpellRequired) {}

  public record Result(boolean success, Failure failure, int manaSpent) {
    private static Result failure(Failure failure) {
      return new Result(false, failure, 0);
    }

    private static Result success(int mana) {
      return new Result(true, Failure.NONE, mana);
    }
  }
  ...
  private static Result begin(Request request, boolean interruptActions) {
    ...
    if (!targetAccepted(spell.getTargetType(), request.targetKind()))
      return Result.failure(Failure.WRONG_TARGET);
    if (request.pvpTarget() && !spell.isPvp()) return Result.failure(Failure.PVP_FORBIDDEN);
    if (request.targetKind() != TargetKind.SELF
        && request.distanceInTiles() > MAXIMUM_CAST_RANGE_TILES) {
      return Result.failure(Failure.OUT_OF_RANGE);
    }
    if (spell.isLineOfSight()
        && request.targetKind() != TargetKind.SELF
        && !request.lineOfSightClear()) {
    ...
    if (caster.isMentallyExhausted() || caster.isStunned())
      return Result.failure(Failure.EXHAUSTED);
    DiceFormula.Context context = context(caster);
    int manaCost =
        caster.hasUnlimitedResources()
            ? 0
