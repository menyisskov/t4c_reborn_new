...
/** Movement-sensitive channel state, isolated from game entities for testing. */
public final class TameChannel {
    public enum Outcome { RUNNING, SUCCESS, INTERRUPTED_MOVED, INTERRUPTED_STUNNED,
        INTERRUPTED_TARGET_LOST, INTERRUPTED_CANCELLED }
    ...
    public TameChannel(float duration, float moveTolerance, Supplier<Vector2> position,
                       BooleanSupplier stunned, BooleanSupplier targetLost) {
    ...
    public Outcome update(float delta) {
        ...
        if (current == null || current.dst(start) > moveTolerance) return outcome = Outcome.INTERRUPTED_MOVED;
        if (targetLost != null && targetLost.getAsBoolean()) return outcome = Outcome.INTERRUPTED_TARGET_LOST;
        elapsed += Math.max(0f, delta);
        if (elapsed >= duration) outcome = Outcome.SUCCESS;
        return outcome;
    }
    public void cancel() { cancelled = true; }
    public float getProgress() { return duration <= 0f ? 1f : Math.min(1f, elapsed / duration); }
}
