...
public final class ChestService {
  ...
  public Result open(Player player, ObjectPos chest, GroundItemManager groundItems) {
    ...
    if (definition == null || definition.getStructure() != 3)
      return new Result(false, Failure.NOT_A_CHEST, null, 0);
    long now = System.currentTimeMillis();
    if (now < nextAvailableAt.getOrDefault(chest, 0L))
      return new Result(false, Failure.COOLDOWN, null, 0);
    ...
    groundItems.spawnFromLoot(
        new LootResult(gold, rewards), chest.x() * GRID_W, chest.y() * GRID_H, player::addGold);
    long delaySeconds =
        Math.max(0L, definition.getLocalRespawn())
  ...
  private static List<String> drawOneFromEachGroup(List<ItemDefinition.ContainerLootGroup> groups) {
    ...
    for (ItemDefinition.ContainerLootGroup group : groups) {
      ...
      if (items != null && !items.isEmpty())
        result.add(items.get(ThreadLocalRandom.current().nextInt(items.size())));
    }
    return result;
  }
}
