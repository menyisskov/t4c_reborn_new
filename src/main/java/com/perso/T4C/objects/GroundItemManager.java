...
public class GroundItemManager {
  ...
  public record CorpseDrop(
      float x, float y, int gold, List<String> itemNames, long createdAtMillis) {}
  ...
  public void setActiveWorld(int worldZ) {
    ...
    WorldDrops target =
        dropsByWorld.computeIfAbsent(
            worldZ, ignored -> new WorldDrops(new ArrayList<>(), new ArrayList<>()));
    items = target.items();
  ...
  public void spawnFromLoot(LootResult loot, float x, float y, IntConsumer goldGained) {
    ...
    log.info(
        "Loot at ({}, {}): {} gold (granted), {} item(s) on ground",
        (int) x,
        (int) y,
        loot.gold(),
        loot.itemNames().size());
  ...
  public CorpseDrop spawnCorpse(
      List<String> itemNames, List<Integer> remainingCharges, int gold, float x, float y) {
    List<String> contents = itemNames == null ? List.of() : List.copyOf(itemNames);
    CorpseDrop corpse =
        new CorpseDrop(x, y, Math.max(0, gold), contents, System.currentTimeMillis());
    corpses.add(corpse);
    for (int i = 0; i < contents.size(); i++) {
      String itemName = contents.get(i);
      int charges =
          remainingCharges != null && i < remainingCharges.size() ? remainingCharges.get(i) : -1;
      if (itemName != null) {
      ...
      item.renderAction =
          () ->
              NameRenderer.renderName(
                  batch, "Corpse", corpse.x() - 16f, corpse.y() - 28f, 32f, 32f);
      out.add(item);
    }
  }
  ...
  public boolean showNameAt(float worldX, float worldY) {
    ...
    for (CorpseDrop corpse : corpses) {
      if (hasRemainingContents(corpse)
          && distanceSquared(worldX, worldY, corpse.x(), corpse.y()) <= 32f * 32f) {
        return true;
      }
    }
    return false;
  ...
  public PickupResult pickUpAt(
      float worldX, float worldY, Player player, int tileDistance, int maxTileDistance) {
    ...
    for (int i = items.size() - 1; i >= 0; i--) {
      ...
      InventoryService.Result result =
          InventoryService.add(player, groundItem.getItemName(), groundItem.getRemainingCharges());
      if (!result.success()) {
    ...
    for (CorpseDrop corpse : corpses) {
      if (corpse.gold() <= 0
          || claimedCorpseGold.contains(corpse)
          || distanceSquared(worldX, worldY, corpse.x(), corpse.y()) > 32f * 32f) continue;
      if (tileDistance > maxTileDistance)
        return new PickupResult(PickupStatus.TOO_FAR, "corpse gold");
