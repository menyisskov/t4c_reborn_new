...
public final class MonsterClanRelations {
  private static final Map<MonsterClan, Set<MonsterClan>> ENEMIES =
      new EnumMap<>(MonsterClan.class);
  ...
  public static MonsterClan resolveClan(String className, String monsterName) {
    String key =
        ((className == null ? "" : className) + " " + (monsterName == null ? "" : monsterName))
    ...
    if (key.contains("cow")
        || key.contains("dromadary")
        || key.contains("pegasus")
        || key.contains("pig")
        || key.contains("unicorn")) {
    ...
    if (key.contains("wasp")
        || key.contains("scorpion")
        || key.contains("spider")
        || key.contains("tarantula")) {
      return MonsterClan.INSECT;
    }
    if (key.contains("cleric")
        || key.contains("guard")
        || key.contains("mage")
        || key.contains("paysan")
        || key.contains("priest")
        || key.contains("thief")
        || key.contains("warrior")
        || key.contains("warrio")
        || key.contains("wizard")
        || key.contains("tank")) {
    ...
    if (key.contains("bat")
        || key.contains("beholder")
        || key.contains("minotaur")
        || key.contains("rat")
        || key.contains("slime")
        || key.contains("snake")
        || key.contains("taunting")
        || key.contains("tree ent")
        || key.contains("treeent")
        || key.contains("troll")
        || key.contains("worm")) {
  ...
  private static void applyRelations(java.util.List<Relation> relations) {
    ...
    for (Relation relation : relations) {
      if (relation == null
          ...
          || relation.source == MonsterClan.NEUTRAL
