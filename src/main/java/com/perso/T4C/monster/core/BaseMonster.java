...
public abstract class BaseMonster implements Nameable {
  ...
  protected com.perso.T4C.monster.loot.LootTable lootTable =
      com.perso.T4C.monster.loot.LootTable.empty();
  ...
  protected com.perso.T4C.npc.companion.CompanionNPC companionAggroTarget = null;
  ...
  @Setter private java.util.function.IntConsumer companionDamageCallback = null;
  ...
  public void teleportTo(float worldX, float worldY) {
  ...
  public void update(float delta, Vector2 playerPosition, List<BaseMonster> nearbyMonsters) {
    ...
    if (isAggro) {
      ...
      updateMonsterCombat(delta, enemyTarget);
        ...
        && distanceToPlayer <= MONSTER_AGGRO_RANGE
  ...
  private boolean isValidMonsterTarget(BaseMonster other) {
    return other != null && other != this && !other.isDead && isEnemyOf(other);
  }
  ...
  private void aggroOnMonster(BaseMonster target) {
    ...
    if (!isAggro || monsterAggroTarget != target) {
      log.info(
          "{} aggroed {} from clan hostility ({} -> {})", name, target.name, clan, target.clan);
      resetChaseNavigation();
    }
    isAggro = true;
    aggroTarget = null;
  ...
  private void updatePatrol(float delta) {
    ...
    if (patrolTarget != null) {
      ...
      if (distance < 2f) {
        ...
        isPaused = true;
        pauseTimer =
            random.nextFloat() * (MONSTER_PATROL_PAUSE_MAX - MONSTER_PATROL_PAUSE_MIN)
                + MONSTER_PATROL_PAUSE_MIN;
        movement.stop();
        patrolTarget = null;
        clearPlannedPath();
      } else {
  ...
  private boolean tryMoveWithFallback(
      float ndx, float ndy, float step, float delta, Vector2 target) {
    if (fallbackTimer > 0f) {
    ...
    if (stuckCooldown <= 0f) {
      ...
      lastFallbackDir.set(fallbackDir);
      log.debug(
          "{} forcing escape dir=({}, {}) for {}s (cooldown {}s)",
          name,
          fallbackDir.x,
          fallbackDir.y,
          String.format("%.2f", fallbackTimer),
          String.format("%.2f", stuckCooldown));
      if (tryMoveAlong(fallbackDir.x, fallbackDir.y, step)) {
        blockedStreak = 0;
        return true;
      }
  ...
  private void recoverBlockedChase(Vector2 target) {
    if (target == null || blockedStreak < BLOCKED_STREAK_BEFORE_REPATH || pathRetryTimer > 0f) {
      return;
    }
  ...
  private boolean canMove(float newX, float newY) {
    ...
    return !collisionManager.isInitialized() || !collisionManager.hasCollision(newX, newY);
  }
  ...
  private void performCompanionAttack(int rawDamage) {
    com.perso.T4C.npc.companion.CompanionNPC target = companionAggroTarget;
    startAttackAnimation();
    com.perso.T4C.combat.CombatResult result =
        com.perso.T4C.combat.CombatResolver.resolve(
            new com.perso.T4C.combat.PhysicalAttackRequest(
                ...
                com.perso.T4C.combat.CombatProfiles.fromCompanion(target),
                rawDamage,
                0,
                false),
            random);
    log.info(
        "{} attacks companion {}: hit={}, damage={}",
        name,
        target.getName(),
        result.hit(),
        result.damage());
  ...
  protected void performAttack(BaseMonster target) {
    ...
    com.perso.T4C.combat.CombatResult result =
        com.perso.T4C.combat.CombatResolver.resolve(
            new com.perso.T4C.combat.PhysicalAttackRequest(
                com.perso.T4C.combat.CombatProfiles.fromMonster(this),
                com.perso.T4C.combat.CombatProfiles.fromMonster(target),
                rawDamage,
                0,
                false),
            random);
  ...
  private int parseDiceRoll(String formula) {
    ...
    java.util.regex.Matcher m =
        java.util.regex.Pattern.compile(
                "(\\d+)d(\\d+)([+-]\\d+)?", java.util.regex.Pattern.CASE_INSENSITIVE)
  ...
  public void exhaustMovementFor(long durationMillis) {
    ...
    movementExhaustedUntilMs =
        Math.max(movementExhaustedUntilMs, System.currentTimeMillis() + durationMillis);
    movement.stop();
  }
  ...
  public int getCombatDodge() {
    int modifier =
        temporaryDodgeModifierUntilMs > System.currentTimeMillis() ? temporaryDodgeModifier : 0;
  ...
  public void applyPlayerDamage(int damage, Player player, XpCurve xpCurve) {
    ...
    takeDamage(damage);
    emitPlayerDamageLifecycle(player);
  ...
  private void emitPlayerDamageLifecycle(Player player) {
    if (player == null || scriptEffectsCallback == null) return;
    if (this instanceof MonsterLifecycle lifecycle) {
  ...
  public void aggroOnCompanion(com.perso.T4C.npc.companion.CompanionNPC companion) {
  ...
  public com.perso.T4C.npc.companion.CompanionNPC getCompanionAggroTarget() {
  ...
  public void render(SpriteBatch batch, ShaderProgram outlineShader) {
    ...
    animations.render(
        ...
        movement.isFlipX(),
        movement.isMoving() || (stationary && shouldAnimateWhileStationary()),
        isHovered || selected,
        outlineShader,
        healthPercent,
        null,
  ...
  public void setScriptPlayer(Player player) {
