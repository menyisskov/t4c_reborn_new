...
class TameChannelTest {
    @Test void completesAtExactDuration() {
    ...
    @Test void interruptionsAreDetectedInPriorityOrder() {
        Vector2 p = new Vector2(); AtomicBoolean stunned = new AtomicBoolean(true);
    ...
    @Test void movementTargetLossAndCancellationInterrupt() {
        Vector2 moved = new Vector2();
        TameChannel movement = new TameChannel(5f, 4f, () -> moved, () -> false, () -> false);
        moved.x = 5; assertEquals(TameChannel.Outcome.INTERRUPTED_MOVED, movement.update(0));
        ...
        cancelled.cancel(); assertEquals(TameChannel.Outcome.INTERRUPTED_CANCELLED, cancelled.update(0));
    }
}
